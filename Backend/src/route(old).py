...
def generateRoute(userInput, nextPlaceSelector, places):
    ...
    while currentTime < endTime:
        ...
        score = score + float(place['rating']) +int(place['user_ratings_total']) * 0.5 - travelTime
    score += len(types)
    route.insert(0,score)
...
def generateFirstPopulation(userInput, populationNumber, places):
    ...
    for _ in range(populationNumber):
        ...
        placesC = newRoute['unselectedPlaces']

    for _ in range(populationNumber):
        newRoute = generateRoute(userInput, getQualitativePlace, placesQ)
        routes.append(newRoute['route'])
        placesQ = newRoute['unselectedPlaces']
...
def removeBadPopulation(population):
    originalNumber = len(population)
    while len(population) != originalNumber/2:
        min = 0
        for i in range(len(population)):
            if population[min][0] > population[i][0]:
                min = i
        del population[min]
    return population
